...
const CitationBar: React.FC<CitationProps> = ({
	...
	const [showCitationSidebarOpen, setShowCitationSidebarOpen] =
		useState(false);
		...
		<div>
			{isDesktopOrLaptop && (
				<div className="h-auto max-h-full overflow-y-auto bg-extralightbg border-4 border-bonewhite p-1 sm:p-2  shadow-inner rounded-md">
					<button
						className="w-full sticky top-0 inline-flex items-center justify-start py-2 sm:py-3 pl-3 sm:pl-4 pr-8 sm:pr-12 overflow-hidden font-montserrat font-semibold text-seasaltwhite transition-all duration-150 ease-in-out rounded hover:pl-8 sm:hover:pl-10 hover:pr-4 sm:hover:pr-6 bg-olivebrown group"
					...
					>
						...
						<span className="absolute left-0 pl-2.5 -translate-x-12 group-hover:translate-x-0 ease-out duration-200">
							...
							{open ? (
								<svg
									className="w-5 h-5 text-olivebrown"
									fill="none"
									stroke="currentColor"
									viewBox="0 0 24 24"
									xmlns="http://www.w3.org/2000/svg"
					...
					<div
						className="overflow-y-auto hide-scrollbar h-full w-25 max-h-full"
						...
						<div className="fixed inset-0 z-40 bg-black/70 flex items-center justify-center shadow-inner">
							<div className="flex justify-center bg-extralightbg border-4 border-bonewhite font-body pb-2">
								<div className=" bg-extralightbg p-2 w-25 shadow-inner rounded-md">
									...
									{/* Citation sidebar content */}
									<div className="h-auto max-h-full overflow-y-auto bg-extralightbg border-4 border-bonewhite p-1 sm:p-2  shadow-inner rounded-md">
										<div className="sticky top-0 inline-flex items-center justify-start py-2 pl-3 pr-3  font-montserrat font-semibold text-seasaltwhite bg-olivebrown">
											Citations
										</div>
										<div
											className="overflow-y-auto hide-scrollbar h-full w-25 max-h-full"
											style={{ maxHeight: "90vh" }}
										>
											{Object.keys(groupedCitations).map(
												(jurisdiction) => (
													<div key={jurisdiction}>
														<div className="pt-2 font-bold">
															{jurisdiction}
