...
export async function jurisdiction_similarity_search_all_partitions(
	...
	const { data, error } = await supabase.rpc(
		`${jurisdiction}_similarity_search`,
		{
...
export async function searchSimilarContent(
	...
	const supabase = createClient(supabaseUrl!, supabaseKey!);
	const { data, error } = await supabase.rpc(
		`${jurisdiction}_searchsimilarcontent`,
		{
			query_embedding: query_embedding,
			match_threshold: match_threshold,
			max_rows: max_rows,
...
export async function searchSimilarStructure(
	...
	const supabase = createClient(supabaseUrl!, supabaseKey!);
	const { data, error } = await supabase.rpc(
		`${jurisdiction}_searchSimilarStructure`,
		{
			query_embedding: query_embedding,
			match_threshold: match_threshold,
			max_rows: max_rows,
...
export async function searchSimilarDefinitions(
	...
	const supabase = createClient(supabaseUrl!, supabaseKey!);
	const { data, error } = await supabase.rpc(
		`${jurisdiction}_searchSimilarDefinitions`,
		{
			query_embedding: query_embedding,
			match_threshold: match_threshold,
			max_rows: max_rows,
...
export async function insertApiUsage(api_usage: APIUsage): Promise<void> {
	const supabase = createClient(
		process.env.NEXT_PUBLIC_SUPABASE_URL!,
		process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
	);
	const { error } = await supabase.from("api_usage").insert(api_usage);
	if (error) {
...
export async function insert_api_debug_log(
	...
	try {
		...
		console.error(
			"Error parsing input or output JSON in insert_api_debug_log!",
		);
		throw error;
	}
	const { error } = await supabase.from("production_api_debug").insert({
		api_phase,
		execution_time,
		input_json,
		output_json,
...
export async function insert_testing_suite_question_results(
	...
	if (error) {
		console.error(
			"Error inserting testing_suite_question_results into database!" +
				error,
		);
		throw error;
	}
