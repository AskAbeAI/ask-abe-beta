...
// Query Scoring
export const scoreQuestionHelper = async (
	base: BaseRequest,
	openai: OpenAI,
	...
	const [response, api_usage] = await createChatCompletionNew(
		params,
		false,
		openai,
	);
	...
	generateLlmReport(
		base,
		"score_question_helper",
		api_usage.error_message || undefined,
	);
...
// Query Expansion
export const expandQueryHelper = async (
	base: BaseRequest,
	openai: OpenAI,
...
export const generateQueryExpansionVitalia = async (
	openai: OpenAI,
	questions: string[],
) => {
	const params = prompts.getPromptExpandedQueryVitalia(questions, true);
	const res = JSON.parse(
		await createChatCompletion(params, openai, "expandedQueryVitalia"),
	);
...
export const generateEmbedding = async (
	openai: OpenAI,
	legal_statements: string[],
) => {
	const embedded_expansion_query = await getEmbedding(
		legal_statements.join("\n"),
		openai,
	);
...
export const generateQueryRefinement = async (
	openai: OpenAI,
	original_question: string,
	clarifying_questions: string[],
	customer_clarifying_responses: string[],
): Promise<[string[], string, string[]]> => {
	const params = prompts.getPromptQueryRefinement(
		original_question,
		clarifying_questions,
		customer_clarifying_responses,
		true,
	);
	const res = JSON.parse(
		await createChatCompletion(params, openai, "queryRefinement"),
	);
...
export const generateBasicQueryRefinement = async (
	openai: OpenAI,
	original_question: string,
) => {
	const params = prompts.getPromptBasicQueryRefinement(
		original_question,
		true,
	);
	const res = JSON.parse(
		await createChatCompletion(params, openai, "basicQueryRefinement"),
	);
	return res;
};
...
export const generateFollowupQuestion = async (
	...
	const params = prompts.getPromptFollowupQuestion(
		question_string,
		already_answered,
		true,
	);
	const response = JSON.parse(
		await createChatCompletion(params, openai, "followupQuestion"),
	);
	console.log(response.followup_question);
	return response.followup_question;
};
export const condenseClarificationsIntoInstructions = async (
	base: BaseRequest,
	openai: OpenAI,
	refinedQuestion: string,
	previousClarifications: Clarification[],
): Promise<string> => {
	const [messages, ragTokens] = prompts.condenseClarifications(
		refinedQuestion,
		previousClarifications,
	); // Ensure expand_query is implemented
...
export const answeringInstructionsHelper = async (
	base: BaseRequest,
	openai: OpenAI,
	refinedQuestion: string,
	...
	const [messages, ragTokens] = prompts.answeringInstructions(
		refinedQuestion,
		alreadyAnswered,
		customerInformation,
	);
...
export const generateNewClarificationQuestion = async (
	openai: OpenAI,
	user_prompt_query: string,
	instructions: string,
): Promise<[Clarification, string]> => {
	const params_choices = prompts.getPromptClarificationQuestion(
		user_prompt_query,
		instructions,
		true,
	);
	const response = JSON.parse(
		await createChatCompletion(
			params_choices,
			openai,
			"queryClarificationQuestion",
		),
	);
	const new_question: string = response.new_clarification.question;
	const new_answers: string[] =
		response.new_clarification.multiple_choice_answers;
	const message_to_customer: string =
		response.new_clarification.message_to_customer;
...
export const generateMultipleClarificationQuestions = async (
	openai: OpenAI,
	user_prompt_query: string,
): Promise<[Clarification[], string[]]> => {
	const params_choices = prompts.getPromptClarificationQuestionMultiple(
		user_prompt_query,
		true,
	);
	const response = JSON.parse(
...
export const directAnswerHelper = async (
	base: BaseRequest,
	openai: OpenAI,
	refinedQuestion: string,
	instructions: string,
	all_text_citation_pairs: text_citation_document_trio[],
): Promise<string> => {
	const [messages, ragTokens] = prompts.directAnswering(
		refinedQuestion,
		instructions,
		all_text_citation_pairs,
	);
...
export const answerNewQuestionHelper = async (
	base: BaseRequest,
	openai: OpenAI,
	refinedQuestion: string,
	instructions: string,
	...
	for (let i = 0; i < rows.length; i++) {
		let current = rows[i]!;
		sectionList.push(
			current.nodeText!.displayTreeAsJson(
				current.number!,
				current.nodeName!,
			),
		);
	...
	const [messages, ragTokens] = prompts.answerNewQuestion(
		refinedQuestion,
		instructions,
		sectionList,
	);
...
export const generateDirectAnswerSimple = async (
	openai: OpenAI,
	user_prompt_query: string,
	instructions: string,
	all_text_citation_pairs: text_citation_pair[],
): Promise<string> => {
	const params = prompts.getPromptDirectAnsweringSimple(
		user_prompt_query,
		instructions,
		all_text_citation_pairs,
		false,
	...
	const result = JSON.parse(
		await createChatCompletion(params, openai, "directAnswering"),
	);
	const direct_answer: string = result.direct_answer;
	return direct_answer;
};

export const generateDirectAnswerVitalia = async (
	openai: OpenAI,
	user_prompt_query: string,
	already_asked_questions: string[],
	all_text_citation_pairs: text_citation_pair[],
): Promise<string> => {
	const params = prompts.getPromptDirectAnsweringVitalia(
		user_prompt_query,
	...
	const result = JSON.parse(
		await createChatCompletion(params, openai, "directAnsweringVitalia"),
	);
	const direct_answer: string = result.direct_answer;
	return direct_answer;
};
