...
export function renderHTML(nodeText: NodeText): string {
    ...
    function renderParagraph(paragraphId: string, nestingLevel: number = 0): string {
        ...
        if (paragraph.references) {
            for (const [key, ref] of Object.entries(paragraph.references.references)) {
    ...
    const topLevelParagraphs = Object.values(paragraphs).filter(p => 
        !p.parent || !paragraphs.hasOwnProperty(p.parent)
    ...
    const htmlContent = topLevelParagraphs.map(index => {
        const paragraphKey = Object.keys(paragraphs).find(key => paragraphs[key].index === index);
