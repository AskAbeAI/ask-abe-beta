...
export async function testingSuite() {
  ...
  //   const rows = await jurisdiction_similarity_search_all_partitions("ca", embedded_expansion_query, 0.8, 20, 60, process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);
  ...
  //   const combined_rows: node_as_row[] = await get_sibling_rows("ca", sibling_node_keys, process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);
  ...
  //   await insert_testing_suite_question_results(question, quality_score, specific_questions, direct_answer, process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!)
