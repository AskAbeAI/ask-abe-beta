...
export async function POST(req: Request) {
	...
	const customer_clarifying_responses: string[] =
		requestData.clarifyingAnswers;

	try {
		const [customer_messages, refined_question, specific_questions] =
			await generateQueryRefinement(
				openai,
				original_question,
		...
		return NextResponse.json({
			errorMessage: `An error occurred in queryRefinement: ${error}`,
		});
	}
}
