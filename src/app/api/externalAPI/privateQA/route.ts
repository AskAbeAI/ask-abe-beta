...
import {
	generateBasicQueryRefinement,
	generateDirectAnswerSimple,
	generateEmbedding,
...
export function OPTIONS(req: Request) {
	...
	const headers = {
		"Access-Control-Allow-Origin":
			"https://concierge-live.vercel.app/chat/", // Modify as needed for your use case
...
export async function POST(req: Request) {
	...
	if (supabaseUrl === undefined) {
		throw new Error("process.env.NEXT_PUBLIC_SUPABASE_URL is undefined!");
	}
	...
	if (
		abe_api_key !== "conciergeTestKey" &&
		abe_api_key !== "seanGroveTestKey"
	) {
		return NextResponse.json({
			errorMessage: `Invalid API key: ${abe_api_key}`,
			status: 401,
		});
	}
	...
	try {
		...
		const embedding: number[] = await generateEmbedding(
			openai,
			legal_questions,
		);
		...
		const primaryRows: node_as_row[] = await callWithRetries(
			() =>
				jurisdiction_similarity_search_all_partitions(
					jurisdiction,
					query_expansion_embedding,
					0.6,
					maxRows,
					supabaseUrl,
					supabaseKey,
		...
		const directAnswer = await directAnswerHelper(
			base,
			openai,
			question,
			instructions,
		...
		return NextResponse.json({
			errorMessage: `Error in privateQA: ${error}`,
			status: 500,
		});
	}
}
