...
export async function POST(req: Request) {
	...
	try {
		if (already_answered.length > 0) {
			let newQuestion = await generateFollowupQuestion(
				openai,
				original_question,
				already_answered,
			);
		...
		const embedding = JSON.parse(
			JSON.stringify(
				await generateEmbedding(openai, [original_question]),
			),
		);

		const rows = await vitalia_search(
			"vitalia",
			embedding,
			0.6,
			50,
		...
		let direct_answer = await generateDirectAnswerVitalia(
			openai,
			original_question,
			already_answered,
			text_citation_pairs,
		...
		if (api_key === "ak_EjMsYGPJpLHcb48r4uCfP2ZYyrjwL") {
			direct_answer = replaceCitationsWithLinks(
				direct_answer,
				citationLinks,
			);
		}
...
function replaceCitationsWithLinks(
	...
	Object.entries(citationLinks).forEach(([citationText, url]) => {
		...
		const escapedCitationText = citationText.replace(
			/[.*+?^${}()'|[\]\\]/g,
			"\\$&",
		);
