...
import {
	getPromptClarificationQuestion,
	getPromptQuerySimilarity,
	getPromptClarificationQuestionMultiple,
	condenseClarifications,
} from "@/lib/prompts";
...
export async function POST(req: Request) {
	...
	try {
		const previous_clarifications: Clarification[] =
			previous_clarifications_raw;

		const params_condensed = getPromptQuerySimilarity(
			user_prompt_query,
			previous_clarifications,
			already_answered,
			true,
		);
		const response_condensed = JSON.parse(
		...
		await insert_api_debug_log(
			"scoreFollowup",
			executionTime,
			JSON.stringify(requestData),
			JSON.stringify(scoreFollowupResponseBody),
			false,
			"",
		...
		return NextResponse.json({
			errorMessage: `An error occurred in scoreFollowup: ${error}`,
		});
	}
}
